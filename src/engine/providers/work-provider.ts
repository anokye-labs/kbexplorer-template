...
export class WorkProvider implements GraphProvider {
  ...
  async resolve(_config: KBConfig, _existingNodes: KBNode[]): Promise<ProviderResult> {
    ...
    for (const pr of this.pullRequests) {
      ...
      const created = new Date(pr.created_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric', timeZone: 'UTC' });
      const updated = new Date(pr.updated_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric', timeZone: 'UTC' });
    ...
    if (this.commits.length > 0) {
      const commitList = this.commits
        ...
        .map(c => {
          ...
          const date = new Date(c.commit.author.date).toLocaleDateString('en-US', { month: 'short', day: 'numeric', timeZone: 'UTC' });
    ...
    for (const release of this.releases) {
      ...
      const pubDate = release.published_at
        ? new Date(release.published_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric', timeZone: 'UTC' })
        : '';
