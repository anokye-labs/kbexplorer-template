...
> **Not to be confused with:** `src/engine/structured-node-map.ts` and its `structured-node-map.yaml` config, a distinct, later-added (F3 / [#166](https://github.com/anokye-labs/kbexplorer-template/issues/166)) module consumed by [`StructuralProvider`](structural-nodes). That one maps *structured config files* (JSON/YAML) to typed JSON-LD nodes by glob/shape rules; this page's `nodemap.ts`/`nodemap.yaml` maps *repo files, directories, and globs* to file/content nodes. The similar names are a known naming collision — check both when searching for "node map" behavior.
